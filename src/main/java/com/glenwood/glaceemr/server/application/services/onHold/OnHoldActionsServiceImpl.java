...
@Service
@Transactional
public class OnHoldActionsServiceImpl implements OnHoldActionsServices{
	...
	@Override
	public List<OnHoldDetailBean> getAllArFollowUP(String fromDate,String toDate){
		...
		cq.distinct(true);
		cq.select(
				builder.construct(OnHoldDetailBean.class,
						...
						insCompAddr1.get(InsCompAddr_.insCompAddrInsName),
						insCompAddr2.get(InsCompAddr_.insCompAddrInsName),
		...
		cq.where(
				builder.equal(cpt.get(Cpt_.cptCpttype),1),
				builder.not(cpt.get(Cpt_.cptCptcode).in("PTSXS","PTREF","INSXS","PT000")),
				//builder.not(root.get(ServiceDetail_.serviceDetailSubmitStatus).in("Y","C","X","Z")),
				builder.equal(root.get(ServiceDetail_.serviceDetailSubmitStatus),"A"),
				builder.notLike(builder.coalesce(/*builder.lower*/(arActionList.get(ArActionList_.arActionListActionName)),"''"),"%missing eob%"),
				builder.between(root.get(ServiceDetail_.serviceDetailPlaceddate),convertToDate(fromDate),convertToDate(toDate))
				);
		//cq.orderBy(builder.asc(root.get(ServiceDetail_.serviceDetailDos)));
		...
		List<OnHoldDetailBean> resultList = new ArrayList<OnHoldDetailBean>();
		for(int i=0;i<results.size();i++)
		{
			OnHoldDetailBean detailsBean=(OnHoldDetailBean) results.get(i);
	...
	public void updateOtherServiceDetail(String popupCurrentStatus, CommonActionBean commonActionBean){
		...
		String query="update associate_service_detail set associate_service_detail_currentstatus = coalesce(associate_service_detail_currentstatus,'') || case when length(associate_service_detail_currentstatus)>0 then ';' else '' end || ' ' || to_mmddyyyy(now()) || ' - "+
				""+Utilities.handleSymbols(HUtil.Nz(popupCurrentStatus, ""))+"'";
